...
>>
Here we use Bayes formula to calculate the probability.
We assume that identical twins must be the same gender, while for fraternal twins this is not a requirement. We also assume that male and female babies are equally likely.

>> Let:    
P(I) = probability of male identical twins    
P(F) = probability of male fraternal twins.

>> Probability of identical twins that are both male = P(I)/(P(I) + P(F))    
>> P(I) = 1/300 x 1/2    
P(F) = 1/125 x 1/4    

>> So using the formulas above, the probability that Elvis had a fraternal twin brother is approximately 0.45.
...
>> Frequentist and Bayesian statistics have several differences. Bayesian statistics involves using a 'prior' probability that will be used in estimation.

>> Frequentists do not require a prior probability, and assume the underlying parameters are fixed. Bayesian statistics are often times more computationally intensive compared to Frequentist statistics. 
